...
| **Restore-from-every-supported-release matrix** | Yes | D-047: for every release **at or above `minimumRestorableVersion`** (D-048), restore a seeded backup of it into `HEAD`, migrate forward, assert schema + domain invariants. Protects the "old backup into new version" promise (F-25) |
